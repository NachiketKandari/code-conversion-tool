...
Scans `.pc`/`.pcf` files and ranks them by conversion complexity. A single file prints one report; a directory walks recursively and produces one CSV row per file. **Name selectors** pin a subset inside a folder — every form is equivalent and mixable, and each name resolves anywhere in the folder's tree (case-insensitive, extension optional):

- `folder/file.pc` — one file, searched within the folder tree
- `folder SVC_DEMO_LIST` — folder + bare name
- `folder "a.pc, b.pc"` / `folder a b c` — comma- **or** space-separated (quoted or as separate arguments)
- `folder list.txt` (or a lone `list.txt`, which evaluates against its own folder) — a file list whose lines carry one-or-more comma/whitespace-separated names, `#` comments skipped

Exactly one match wins everywhere; duplicates collapse; zero or several matches, or any miss, are loud errors naming the candidates/list — never a silent pick:
...
go run ./cmd/tuxgo analyze testdata/nav "svc_demo_list, fn_demo_lib"
go run ./cmd/tuxgo analyze testdata/nav my-picks.txt          # file list (one or more names per line)
...
- **tpcall sites** — each `tpcall` call site correlates into a `TPCall`
  fact: the outbound service name plus its FML contract — `Fadd32` ops into the send buffer before the call, `Fget32` ops from the receive buffer after (the enclosing block is the window); sites whose buffers can't be correlated degrade to a visible `ambiguous` fact. **The call's target file resolves corpus-wide** in dir mode: the `.pc` file(s) whose entry function or base name matches the service name (`tpcall("SVC_MIN_DETAIL", …)` → `service_file: …/SVC_MIN_DETAIL.pc`, comma-joined on multiple matches, case-insensitive); single-file extraction has no corpus and a service outside the scanned corpus keeps the field empty (WARN in the run log) — the target rides the extract JSON, the extract log, the generated placeholder stub (`// target: …`), and the controller prompt's placeholder signature
...
- **defines + error codes** — every `#define`/`#undef` is a `defines` fact
  (name, raw value, line, scope: file or enclosing function; function-like macros are recorded `macro: true` and never substituted; `#undef` is a tombstone). Predicate trees resolve a name when its define chains to a pure literal (`6144`, `'H'`, `"msg"`); compound values (`(BUF_LEN*3)`) keep the name and ride the controller prompt's `Legacy constants` section instead. Every `Fadd32` also carries a `code` when the legacy convention filled its message variable — the nearest preceding `errlog`/`strcpy`/`sprintf` writer's literal (the nearest writer wins; a post-add writer never counts, an unresolvable code stays honestly empty) or the add's own string literal — the codes the legacy runtime's message map turns into real errors
...
- **`-go` draft**: the deterministic transpilation draft — guards collapse, fetch loops become `rows, err := <store call>` + `for _, row := range rows`, Tuxedo buffer management/logging elides with a count, and everything untranspilable becomes an explicit `// TODO(line N):` marker (a correct skeleton, never a guess). `#define` constants resolve in if/while conditions when their value is a pure literal (`#define FLAG_H 'H'` → `if c_flag == "H"`; compound values keep the name), define/undef lines classify instead of counting as residue, and `tpreturn(TPFAIL, …)` lines carry `// legacy error code(s): S31005, …` — the codes the legacy runtime maps to real messages;
...
The scan keeps the human decision (§4.2.8 — the tool never invents endpoints): it finds the **API candidates** — conditions/blocks enclosing `Fget32` request reads **and** non-error `Fadd32` response writes (error emissions — `FML_ERR_MSG` into the input buffer, "fadd err = returning error" — never count; an if enclosing only error emission is a guard, not an API), including qualifying nested ifs (keyed `c<n>.<k>`; subsets of their parent are marked redundant), and emits an **editable mapping draft** per entry: endpoints with pre-filled `name`/`route` (always editable), per-candidate read/write/query census comments plus the legacy error codes each candidate ships, non-candidates kept commented-out for control, `service` prefilled, and a `dbMethods:` block (AI-proposed pins, or the commented skeleton listing the entry's query IDs). `module:` defaults to the service name at load, so a draft is **loadable as-is** — reviewing the suggested names is the only step, and even that is optional. Compound conditions (`a == 'N' || a == 'K' && …`) parse with full C precedence; a flag-if with no FML I/O inside an API stays that API's inner conditional, while an inner if with its own reads+writes is a taggable nested candidate.
